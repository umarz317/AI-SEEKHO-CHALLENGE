// app/trace.js — Screen 6: Agent Trace (integrated)
...
export default function TraceScreen() {
  ...
  const traceEvents = apiResult?.trace || MDATA.trace;
  const traceId = apiResult?.traceId || 'TRC-20260517-001';
  const stepCount = traceEvents.length;

  return (
    <View style={{ flex: 1, backgroundColor: M.bg }}>
      <TopBar title="Agent trace" subtitle={`${stepCount} steps · all successful`} onBack={() => router.back()} />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 14, paddingBottom: 24 }}>
        <View style={{ backgroundColor: M.primary, borderRadius: 16, padding: 16, marginBottom: 14 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
            ...
            <Text style={{ fontSize: 11, fontWeight: '700', color: M.accent, textTransform: 'uppercase', letterSpacing: 0.8 }}>Agentic Workflow</Text>
          </View>
          <Text style={{ fontSize: 18, fontWeight: '800', color: '#fff', letterSpacing: -0.2, marginBottom: 4 }}>
            {stepCount} agents · {stepCount} tools
          </Text>
          ...
          <View style={{ backgroundColor: 'rgba(255,255,255,0.07)', borderRadius: 8, padding: 10, borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.8 }}>Trace ID</Text>
            <Text style={{ fontSize: 11, fontWeight: '600', color: '#fff' }}>{traceId}</Text>
          </View>
        </View>

        <View>
          {traceEvents.map((ev, i) => (
            <View key={i} style={{ flexDirection: 'row', gap: 12, paddingBottom: i < traceEvents.length - 1 ? 14 : 0 }}>
              <View style={{ alignItems: 'center' }}>
                <View style={{ width: 34, height: 34, borderRadius: 10, backgroundColor: ev.color, alignItems: 'center', justifyContent: 'center',
              ...
              <MCard style={{ flex: 1, marginBottom: 0, padding: 12 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 5 }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: M.successBg, borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 }}>
                ...
                <View style={{ backgroundColor: M.surfaceLow, borderRadius: 6, padding: 8, borderWidth: 1, borderColor: M.divider }}>
